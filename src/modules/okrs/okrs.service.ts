...
export class OkrsService {
  ...
  async findAll(query: QueryOkrsDto) {
    const { page = 1, limit = 10, user_id, assessment_id, status, search } = query;
    ...
    if (assessment_id) {
      queryBuilder.andWhere('okr.assessment_id = :assessment_id', { assessment_id });
    }
    ...
    if (search) {
      queryBuilder.andWhere('okr.objective LIKE :search', { search: `%${search}%` });
    }

    queryBuilder
      .orderBy('okr.created_at', 'DESC')
      .skip(skip)
      .take(limit);
  ...
  async create(createOkrDto: CreateOkrDto): Promise<Okr> {
    ...
    try {
      ...
      // 验证关键结果权重总和是否为100%
      const totalWeight = createOkrDto.key_results.reduce((sum, kr) => sum + kr.weight, 0);
  ...
  async updateKeyResult(okrId: number, keyResultId: number, updateKeyResultDto: UpdateKeyResultDto): Promise<KeyResult> {
