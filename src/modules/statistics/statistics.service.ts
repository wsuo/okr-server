...
export class StatisticsService {
  ...
  async getAssessmentStatistics(query: StatisticsQueryDto) {
    ...
    if (start_date) {
      queryBuilder.andWhere('assessment.start_date >= :start_date', { start_date });
    }
    if (end_date) {
  ...
  async getDepartmentStatistics() {
    ...
    return result.map(item => ({
      ...
      self_completion_rate: item.participant_count > 0 ? 
        (parseInt(item.self_completed) / parseInt(item.participant_count)) * 100 : 0,
      leader_completion_rate: item.participant_count > 0 ? 
        (parseInt(item.leader_completed) / parseInt(item.participant_count)) * 100 : 0,
    }));
  }
  ...
  async getEvaluationStatistics(query: StatisticsQueryDto) {
    ...
    if (start_date) {
      queryBuilder.andWhere('evaluation.submitted_at >= :start_date', { start_date });
    }
    if (end_date) {
      queryBuilder.andWhere('evaluation.submitted_at <= :end_date', { end_date });
    }
    if (department_id) {
      queryBuilder.andWhere('department.id = :department_id', { department_id });
    }
  ...
  async getPerformanceTrends(query: StatisticsQueryDto) {
    const { start_date, end_date, time_dimension = 'month', department_id } = query;
