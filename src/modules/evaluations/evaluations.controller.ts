...
export class EvaluationsController {
  ...
  @ApiResponse({ status: 200, description: '获取成功', type: EvaluationTemplateResponseDto })
  ...
  @ApiResponse({ status: 200, description: '获取成功', type: UserEvaluationTemplateDto })
  ...
  getUserEvaluationTemplate(
    ...
    return this.evaluationsService.getUserEvaluationTemplate(+assessmentId, +userId, currentUser.id);
  }
  ...
  createDetailedSelfEvaluation(
    ...
    return this.evaluationsService.createDetailedSelfEvaluation(createDetailedSelfEvaluationDto, user.id);
  }
  ...
  createDetailedLeaderEvaluation(
    @Body() createDetailedLeaderEvaluationDto: CreateDetailedLeaderEvaluationDto,
    @CurrentUser() user: any
  ) {
    return this.evaluationsService.createDetailedLeaderEvaluation(createDetailedLeaderEvaluationDto, user.id);
  }
  ...
  saveDraft(
    ...
    return this.evaluationsService.saveDraft(+id, saveEvaluationDraftDto, user.id);
  }
  ...
  @ApiResponse({ status: 200, description: '获取成功', type: EvaluationProgressDto })
  @ApiResponse({ status: 404, description: '考核不存在' })
  getEvaluationProgress(
    @Param('assessmentId') assessmentId: string,
    @CurrentUser() user: any
  ) {
    return this.evaluationsService.getEvaluationProgress(+assessmentId, user.id);
  }

  @Get('subordinates/:assessmentId')
  @ApiOperation({ summary: '获取下属评分任务' })
  @ApiResponse({ status: 200, description: '获取成功', type: [SubordinateEvaluationTaskDto] })
  @ApiResponse({ status: 404, description: '考核不存在' })
  ...
  getEvaluationComparison(
    ...
    return this.evaluationsService.getEvaluationComparison(+assessmentId, +userId, user.id);
  }
}