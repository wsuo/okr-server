...
export class AssessmentsController {
  ...
  @ApiResponse({ status: 200, description: '校验成功', type: EndValidationResponseDto })
  @ApiResponse({ status: 404, description: '考核不存在' })
  ...
  @ApiResponse({ status: 200, description: '校验成功', type: DeleteValidationResponseDto })
  @ApiResponse({ status: 404, description: '考核不存在' })
  ...
  @ApiResponse({ status: 200, description: '获取成功', type: ScorePreviewResponseDto })
  @ApiResponse({ status: 404, description: '考核不存在' })
  async getScorePreview(@Param('id') id: string): Promise<ScorePreviewResponseDto> {
  ...
  editAssessment(
    ...
    return this.assessmentsService.editAssessment(+id, editAssessmentDto, user.id);
  }
