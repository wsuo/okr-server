...
export class DepartmentsService {
  ...
  async create(createDepartmentDto: CreateDepartmentDto): Promise<Department> {
    ...
    const existingDepartment = await this.departmentsRepository
      ...
      .andWhere('dept.parent_id = :parent_id', { parent_id: createDepartmentDto.parent_id || null })
  ...
  async update(id: number, updateDepartmentDto: UpdateDepartmentDto): Promise<Department> {
    ...
    if (updateDepartmentDto.parent_id !== undefined) {
      ...
      if (updateDepartmentDto.parent_id) {
        ...
        // 检查是否会形成循环引用
        const isCircular = await this.checkCircularReference(id, updateDepartmentDto.parent_id);
        if (isCircular) {
          throw new BadRequestException('不能设置子部门为父部门，会形成循环引用');
        }
      }
    }

    // 检查同级部门名称是否重复（如果要修改名称或父部门）
    if (updateDepartmentDto.name || updateDepartmentDto.parent_id !== undefined) {
      const name = updateDepartmentDto.name || department.name;
      const parent_id = updateDepartmentDto.parent_id !== undefined ? updateDepartmentDto.parent_id : department.parent_id;

      const existingDepartment = await this.departmentsRepository
        .createQueryBuilder('dept')
        .where('dept.name = :name', { name })
        .andWhere('dept.parent_id = :parent_id', { parent_id: parent_id || null })
  ...
  private async checkCircularReference(deptId: number, parentId: number): Promise<boolean> {
    if (deptId === parentId) {
      return true;
    }
